...
v0.3.0-rc.1 adds `TabsList`, `TabsTrigger`, and `TabsContent` subcomponents. The old `Items` / `TabItems` data-driven API still works, but the compositional pattern is now recommended for full markup control.
