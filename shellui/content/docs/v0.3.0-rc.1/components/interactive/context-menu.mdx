...
`ContextMenu` is **new in v0.3.0-rc.1**. Wrap any element — it will show a right-click floating menu. Menu items are passed as a `List<ContextMenuItem>` via the `Items` parameter. The wrapped `ChildContent` is the right-click target.
