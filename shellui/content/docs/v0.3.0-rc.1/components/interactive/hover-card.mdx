...
v0.3.0-rc.1 keeps the same `<ChildContent>` + `<CardContent>` render fragment API, with `ChildContent` being the hover trigger and `CardContent` being the floating card body.
