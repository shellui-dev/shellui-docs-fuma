...
v0.3.0-rc.1 adds `SheetTrigger` and `SheetContent` subcomponents. `Side` is set on `<Sheet>` itself. Use `@bind-Open` when you need to control open state externally (e.g. a close button inside the content).
