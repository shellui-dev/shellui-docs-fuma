...
v0.3.0-rc.1 introduces `PopoverTrigger` and `PopoverContent` subcomponents. The old `IsOpen` / `Trigger` render fragment API still works but the compositional pattern is now recommended.
