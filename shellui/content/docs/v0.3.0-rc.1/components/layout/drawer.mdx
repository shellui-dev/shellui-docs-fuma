...
v0.3.0-rc.1 adds `DrawerTrigger` and `DrawerContent` subcomponents. `Side` is set on `<Drawer>` itself. You can also bind `@bind-Open` to control open state externally — useful when you need a close button inside the content.
