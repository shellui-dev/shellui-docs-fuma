...
v0.3.0-rc.1 refactors Accordion to a fully compositional subcomponent model. The old `Title` parameter on `AccordionItem` still works, but the new `AccordionTrigger` / `AccordionContent` pattern is recommended. A new `AccordionType` enum controls single vs multiple open mode.
