---
title: Select
description: Dropdown select input for picking from a list of options, styled to match the ShellUI design system.
---

`Select` renders a styled native `<select>` element. `SelectTrigger`, `SelectContent`, and `SelectItem` sub-components are planned for a future release and are not yet implemented in v0.3.0-rc.1.
