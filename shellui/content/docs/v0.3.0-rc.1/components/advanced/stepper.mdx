...
v0.3.0-rc.1 overhauled the Stepper. `@bind-CurrentStep` is now `@bind-Value`, and a new **compositional mode** lets you place step headers and content manually. The old data-driven `StepItems` mode still works.
...
Active-state highlighting may not always reflect the current step in some edge cases. This is a known issue shipped as-is in v0.3.0-rc.1.
</Callout>
